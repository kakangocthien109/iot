...
float toa_do[6]; // Mang chua toa do ax, ay, bx, by, cx, cy
float canh_goc[6]; // Mang chua cac canh va goc, thu tu: canh c, b, a, goc A, B, C
float dc[3]; // Duong cao tam giac
float tt[3]; // Trung tuyen tam giac
float p; // Nua chu vi TG cho cong thuc Heron (dung 2 lan)

void duongcao_tamgiac()
{
  p = (canh_goc[2] + canh_goc[1] + canh_goc[0]) / 2.0; // Tinh nua chu vi

  // Ap dung cong thuc Heron, tinh do dai duong cao tam giac
...
void dientich_tamgiac()
{
  ...
  // Ap dung tiep cong thuc Heron, tinh DT theo cong thuc Heron
...
int main()
{
  do
  {
    ...
    if (!(check_tamgiac(toa_do[0], toa_do[1], toa_do[2], toa_do[3], toa_do[4], toa_do[5]))) // Neu gia tri tinh duoc cua cac toa do khong bang ham check_tamgiac => khong phai 1 tam giac
    {
